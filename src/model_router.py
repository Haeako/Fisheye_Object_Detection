...
config = load_config('../config/router.yaml')
RUNTIME = config['runtime_engine']
...
class ModelEnsembleRouter:
    def __init__(self, models_folder, max_workers=4):
        self.dfine_models = []
        RUNTIME = config.get('runtime_engine', 'ONNX')
        ...
        # self.class_specific_configs = config['class_specific_configs']
        ...
        self.dfine_thresholds_config = config['dfine_thresholds_config']
    ...
    def predict_single_model(self, model, img, class_name=None):
    ...
    def perform_wbf(self, results_list, image_shape, iou_thr=0.5, skip_box_thr=0.1):
        ...
        for result in results_list:
            ...
            if boxes.numel() > 0:
                ...
                boxes_norm = boxes / torch.tensor([width, width, height, height], device=boxes.device)
